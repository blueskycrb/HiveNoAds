...
**v1.2**：多地址下载真实视频 → 校验可播放后再写入相册（`mediaType=Video`）。若只找到封面会明确提示，不会假装“视频已保存”。悬浮按钮从整个窗口收集播放地址（不再优先封面图）。


**优先尝试解锁原生下载门控**；仍不可用时用右侧青色 **↓** 悬浮按钮或双指长按兜底。

1. 下载 [Releases](../../releases) 里的 **`Douyin.dylib`**（ASCII 名，避免 GitHub 把中文资源改成 `default.dylib`）
